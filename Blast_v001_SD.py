...
# Custom dialog that uses a QListWidget (for multi-selection of groups)
# and a read-only QLineEdit for displaying the current target node.
class GroupSelectionDialog(QtWidgets.QDialog):
    ...
    def autoRefresh(self):
        # Fetch current selection; if none, clear the node field and group list.
...
# Blast node creation function using Houdini's automatic placement,
# preserving color gradient assignment.
def blast_groups_from_selected(selected_groups):
    ...
    for i, group in enumerate(selected_groups):
        ...
        red, green, blue = 1.0, 1.0, fade_factor * 0.8
        blast.setColor(hou.Color((red, green, blue)))
        # Schedule Houdini's automatic placement.
        QtCore.QTimer.singleShot(0, lambda b=blast: b.moveToGoodPosition())

# Global variable to hold the dialog reference.
gDialog = None

# Show the dialog modelessly (without requiring an initial selection).
